...
# -----------------------------------------------------------------------------
# WEBRTC ICE CONFIGURATION (THE MISSING PIECE)
# -----------------------------------------------------------------------------
# Without this, the browser and the Streamlit Cloud server have no way to
# discover a public address to connect through once they're on different
# networks. Google's public STUN server fixes this for most home/office
# networks. If you're still on a restrictive network (school/corporate
# firewall blocking P2P UDP), swap this for a Twilio TURN config instead.
...
# #                 rx1 = int(max(0, bbox.xmin * w + (bbox.width * w * 0.25)))
# #                 rx2 = int(min(w, bbox.xmin * w + (bbox.width * w * 0.75)))
# #                 ry1 = int(max(0, bbox.ymin * h))
# #                 ry2 = int(min(h, bbox.ymin * h + (bbox.height * h * 0.35)))
...
# #                         app_state.face_photo = img[face_y:face_y+face_h, face_x:face_x+face_w].copy()
...
# #             # Peak detection for HRV
# #             peaks, _ = self._detect_peaks(filtered_signal, actual_fs)
# #             if len(peaks) > 2:
# #                 rr_intervals = np.diff(peaks) / actual_fs  # in seconds
# #                 app_state.rr_intervals = rr_intervals * 1000  # convert to ms
...
# #                     # Stress Index (simplified: based on HRV and HR)
# #                     # Lower HRV = Higher stress
# #                     normalized_hrv = np.clip(app_state.hrv / 100.0, 0, 1)  # Normalize to 0-1
# #                     app_state.stress_index = (1 - normalized_hrv) * 100  # 0-100 scale
...
# #     def _detect_peaks(self, signal, fs, min_distance=0.5):
# #         """Simple peak detection for heartbeats"""
# #         from scipy.signal import find_peaks
# #         min_samples = int(min_distance * fs)
# #         peaks, _ = find_peaks(signal, distance=min_samples, height=0)
...
# #         with col_a:
# #             st.markdown(f"""
# #             <div class="metric-card" style="border-bottom: 4px solid #EC4899;">
# #                 <div class="metric-label">Heart Rate</div>
# #                 <div class="metric-value" style="color: #EC4899;">{app_state.bpm:.0f}</div>
...
# #         with col_c:
# #             st.markdown(f"""
# #             <div class="metric-card" style="border-bottom: 4px solid #06B6D4;">
# #                 <div class="metric-label">Respiration</div>
# #                 <div class="metric-value" style="color: #06B6D4;">{app_state.rr:.1f}</div>
...
# #             st.markdown(f"""
# #             <div class="metric-card" style="border-bottom: 4px solid {stress_color};">
# #                 <div class="metric-label">Stress Index</div>
# #                 <div class="metric-value" style="color: {stress_color};">{app_state.stress_index:.0f}</div>
# #                 <div class="metric-unit">{stress_label}</div>
# #             </div>
# #             """, unsafe_allow_html=True)
...
# #         st.markdown("---")
# #         if len(app_state.data_log) > 30:  # At least 30 seconds of data
# #             pdf_buffer = generate_pdf_report()
# #             st.download_button(
# #                 label="📄 Download Medical Report (PDF)",
# #                 data=pdf_buffer,
...
# #     Beats per minute. Normal resting range: 60-100 BPM.
    
# #     *Measured via FFT analysis of green light absorption in facial capillaries.*
# #     """)
...
# #     Time variation between heartbeats. Higher is better (20-100ms normal).
    
# #     *Indicates autonomic nervous system health and stress resilience.*
# #     """)
...
# #     Breaths per minute. Normal range: 12-20 BrPM.
    
# #     *Extracted from baseline wander in the PPG signal (0.15-0.5 Hz).*
# #     """)
...
# #     0-100 scale based on HRV analysis. Lower is better (<50 optimal).
    
# #     *Derived from sympathetic/parasympathetic nervous system balance.*
# #     """)
