...
class rPPGProcessor(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        if len(faces) > 0:
            ...
            if rx2 > rx1 and ry2 > ry1:
                ...
                # Maintain a 25-second rolling buffer. 15 seconds turned out
                # to be a hard ceiling on how much data HRV peak-detection
                # ever had to work with — once the buffer filled, waiting
                # longer in the scan didn't help, since old samples were
                # already being dropped as fast as new ones came in. 25
                # seconds gives meaningfully more beats to work with.
                if len(self.green_signal) > int(self.fps * 25):
    ...
    def _calculate_all_metrics(self):
        ...
        if len(valid_idx) > 0:
            ...
            # removes a few, at least 4 clean intervals can survive —
            # diffing N peaks always yields N-1 intervals, so the raw
            # requirement must leave real headroom above the final one.
            # With the buffer now holding up to 25 seconds of signal, 6+
            # raw peaks is realistic even at a fairly slow heart rate.
            if len(peaks) >= 6:
                ...
                # Require enough surviving intervals for a real estimate — 4
                # is a reasonable floor for statistical grounding. Combined
                # with the 6-peak raw requirement above, this is now
                # mathematically achievable rather than an impossible bar.
